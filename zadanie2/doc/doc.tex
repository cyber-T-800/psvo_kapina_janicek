...
\subsection{Demonštrácia na reálnom obraze}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{undisortion_demonstacia.png}
    \caption{Vľavo: pôvodný obraz, vpravo: obraz po odstránení disorzie (\texttt{cv2.undistort})}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{detekcia_tvarov_demonstacia.png}
    \caption{Demonštrácia detekcie geometrických tvarov.}
\end{figure}

\newpage
\section{Farebný filter}

Pre zmenu jednej farby na iný sme najskôr zmenili farebný priestor z pôvodného RGB z kamery na HSV pomocou funkcie \texttt{cv2.cvtColor}. Následne sme použili funkciu \texttt{cv2.inRange} na vytvorenie masky, ktorá vyberá pixely v zadanom rozsahu farieb v HSV priestore. Nakoniec sme všetky pixely, ktoré patrili do masky, zmenili na požadovanú farbu.

\subsection{Demonštrácia na reálnom obraze}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{filter_demonstacia.png}
    \caption{Demonštrácia farebného filtra. Vľavo: pôvodný obraz, vpravo: obraz po aplikovaní farebného filtra.}
\end{figure}
